...
class PerformanceAnalyzer:
    ...
    def analyze_project_performance(self, project_path: str = None) -> PerformanceReport:
        ...
        for py_file in python_files:
            try:
                ...
                logger.warning(f"Erreur lors de l'analyse de performance de {py_file}: {e}")
    ...
    def _analyze_file_performance(self, file_analysis: FileAnalysis) -> List[PerformanceMetric]:
        ...
        complexity_metric = PerformanceMetric(
            ...
            status=self._get_metric_status(file_analysis.complexity_score, self.thresholds["complexity"])
        )
        ...
        size_metric = PerformanceMetric(
            ...
            location=str(file_analysis.file_path),
            threshold=500,
            status=self._get_metric_status(file_analysis.total_lines, 500, reverse=True)
        )
        ...
        imports_metric = PerformanceMetric(
            ...
            status=self._get_metric_status(len(file_analysis.imports), self.thresholds["imports"])
        )
        ...
        for func in file_analysis.functions:
            func_metric = PerformanceMetric(
                ...
                status=self._get_metric_status(func.complexity, self.thresholds["function_size"])
            )
            metrics.append(func_metric)
        
        return metrics
    
    def _detect_performance_issues(self, file_analysis: FileAnalysis) -> List[PerformanceIssue]:
        ...
        for func in file_analysis.functions:
            if func.complexity > 8:  # Seuil plus bas
                issue = PerformanceIssue(
                    ...
                    location=f"{file_analysis.file_path}:{func.line_number}",
                    description=f"Fonction {func.name} très complexe: {func.complexity}",
                    impact="high",
    ...
    def _get_metric_status(self, value: float, threshold: float, reverse: bool = False) -> str:
    ...
    def _generate_performance_recommendations(self, issues: List[PerformanceIssue]) -> List[str]:
        ...
        if "high_complexity" in issue_types:
            ...
            recommendations.append(f"🧠 {count} modules très complexes - priorité au refactoring")
        
        if "complex_function" in issue_types:
            count = len(issue_types["complex_function"])
            recommendations.append(f"⚙️ {count} fonctions complexes - diviser en sous-fonctions")
        
        if "large_file" in issue_types:
            count = len(issue_types["large_file"])
            recommendations.append(f"📄 {count} fichiers très longs - diviser en modules")
        
        if "excessive_imports" in issue_types:
            count = len(issue_types["excessive_imports"])
            recommendations.append(f"📦 {count} fichiers avec trop d'imports - nettoyer")
        
        # Recommandations générales
        if issues:
            total_improvement = sum(issue.estimated_improvement for issue in issues)
            recommendations.append(f"📈 Amélioration potentielle estimée: {total_improvement:.1f}%")
        
        return recommendations
    
    def _identify_optimization_opportunities(self, issues: List[PerformanceIssue]) -> List[str]:
        """Identifier les opportunités d'optimisation"""
        opportunities = []
        ...
        size_issues = [i for i in issues if "size" in i.issue_type or "large" in i.issue_type]
        if size_issues:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for metric in report.metrics:
                cursor.execute("""
                    ...
                    (metric_type, value, unit, location, threshold, status, measured_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    metric.metric_type,
                    metric.value,
            ...
            for issue in report.issues:
                cursor.execute("""
                    ...
                    (issue_type, location, description, impact, suggestion, estimated_improvement, detected_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    issue.issue_type,
                    issue.location,
    ...
    def profile_function(self, function_path: str, function_name: str, *args, **kwargs) -> Dict[str, Any]:
    ...
    def get_performance_insights(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("SELECT AVG(value) FROM performance_metrics WHERE metric_type = 'complexity'")
            avg_complexity = cursor.fetchone()[0] or 0
            
            cursor.execute("SELECT COUNT(*) FROM performance_issues WHERE resolved_at IS NULL")
            ...
            return {
                ...
                "performance_health": max(0, 100 - (avg_complexity * 2 + unresolved_issues * 5))
            } 