...
class ConfigManager:
    ...
    def _merge_yaml_config(self, config: AthaliaConfig, yaml_data: Dict[str, Any]) -> AthaliaConfig:
        ...
        if 'database' in yaml_data:
            ...
            config.db_backup_retention=db.get('backup_retention', config.db_backup_retention)
        ...
        if 'ai' in yaml_data:
            ...
            config.ai_fallback_enabled=ai.get('fallback_enabled', config.ai_fallback_enabled)
        ...
        if 'cicd' in yaml_data:
            cicd=yaml_data['cicd']
            config.cicd_github_actions=cicd.get('github_actions', config.cicd_github_actions)
        ...
        if 'cleanup' in yaml_data:
            ...
            config.cleanup_auto_clean=cleanup.get('auto_clean', config.cleanup_auto_clean)
        ...
        if 'dashboard' in yaml_data:
            ...
            config.dashboard_auto_generate=dashboard.get('auto_generate', config.dashboard_auto_generate)
            ...
            config.dashboard_auto_open=dashboard.get('auto_open', config.dashboard_auto_open)

        # Profils
        if 'profiles' in yaml_data:
            profiles=yaml_data['profiles']
            config.profiles_auto_create=profiles.get('auto_create', config.profiles_auto_create)
            config.profiles_default_user=profiles.get('default_user', config.profiles_default_user)
            config.profiles_history_retention=profiles.get('history_retention', config.profiles_history_retention)

        # Sécurité
        if 'security' in yaml_data:
            security=yaml_data['security']
            config.security_audit_enabled=security.get('audit_enabled', config.security_audit_enabled)
            config.security_secrets_detection=security.get('secrets_detection', config.security_secrets_detection)
            config.security_vulnerability_scan=security.get('vulnerability_scan', config.security_vulnerability_scan)

        # Analytics
        if 'analytics' in yaml_data:
            analytics=yaml_data['analytics']
            config.analytics_enabled=analytics.get('enabled', config.analytics_enabled)
            config.analytics_metrics_retention=analytics.get('metrics_retention', config.analytics_metrics_retention)
            config.analytics_auto_export=analytics.get('auto_export', config.analytics_auto_export)
    ...
    def _merge_env_config(self, config: AthaliaConfig) -> AthaliaConfig:
        ...
        config.verbose=os.getenv('ATHALIA_VERBOSE', str(config.verbose)).lower()=='true'
        config.auto_fix=os.getenv('ATHALIA_AUTO_FIX', str(config.auto_fix)).lower()=='true'
        config.dry_run=os.getenv('ATHALIA_DRY_RUN', str(config.dry_run)).lower()=='true'
