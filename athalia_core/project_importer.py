"""
Module d'import intelligent pour projets existants.
...
class ProjectImporter:
    ...
    def _detect_project_type(self, project_path: str, structure: Dict[str, Any]) -> str:
        ...
        file_names = [f.lower() for f in structure['files']]
    ...
    def _analyze_code_quality(self, project_path: str) -> Dict[str, Any]:
        ...
        analysis['has_tests'] = any(f.lower().endswith('.py') for f in files)
        analysis['has_docs'] = any(f.endswith('.md') for f in files)
        analysis['has_requirements'] = 'requirements.txt' in files
        analysis['has_readme'] = 'README.md' in files
        
        # Compter les fichiers Python
        for root, dirs, files in os.walk(project_path):
            analysis['python_files_count'] += len([f for f in files if f.endswith('.py')])
            analysis['test_files_count'] += len([f for f in files if 'test' in f.lower()])
        
        # Détecter les problèmes
        if not analysis['has_tests']:
            analysis['issues'].append("Aucun test détecté")
        if not analysis['has_docs']:
            analysis['issues'].append("Documentation manquante")
        if not analysis['has_requirements']:
            analysis['issues'].append("Fichier requirements.txt manquant")
        if not analysis['has_readme']:
            analysis['issues'].append("README.md manquant")
    ...
    def _generate_correction_blueprint(self, project_path: str, structure: Dict[str, Any], 
        ...
        blueprint = {
            'project_name': f"{project_name}_enhanced",
    ...
    def _suggest_structure(self, structure: Dict[str, Any]) -> List[str]:
        ...
        if not any('doc' in f.lower() for f in structure['files']):
    ...
    def _suggest_enhancements(self, project_type: str, quality_analysis: Dict[str, Any]) -> List[str]:
        ...
        if not quality_analysis['has_tests']:
            enhancements.append("Ajouter une suite de tests complète")
        if not quality_analysis['has_docs']:
            enhancements.append("Générer une documentation technique")
        ...
        if not quality_analysis['has_readme']:
            enhancements.append("Créer un README.md détaillé")
        # Améliorations spécifiques au type
        if project_type == 'api':
            enhancements.append("Ajouter une authentification JWT")
            enhancements.append("Implémenter une validation des données")
        elif project_type == 'game':
            enhancements.append("Ajouter un système de score")
            enhancements.append("Implémenter des effets sonores")
        elif project_type == 'ai':
            enhancements.append("Ajouter un système de cache")
            enhancements.append("Implémenter un monitoring des performances")
        return enhancements
