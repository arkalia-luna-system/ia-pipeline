...
class IntelligentMemory:
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Prédictions
            cursor.execute("SELECT COUNT(*) FROM predictions WHERE validated = 1")
            validated_predictions = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM predictions WHERE validated = 1 AND validation_result = 'correct'")
            correct_predictions = cursor.fetchone()[0]
            
            # Corrections suggérées
            cursor.execute("SELECT COUNT(*) FROM correction_suggestions WHERE applied = 1")
            applied_corrections = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM correction_suggestions WHERE applied = 1 AND success = 1")
            successful_corrections = cursor.fetchone()[0]
            
            # Calculer les taux de succès
            error_rate = total_errors / total_events if total_events > 0 else 0
            prediction_accuracy = correct_predictions / validated_predictions if validated_predictions > 0 else 0
            correction_success_rate = successful_corrections / applied_corrections if applied_corrections > 0 else 0
