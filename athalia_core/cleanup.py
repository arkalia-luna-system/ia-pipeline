...
def clean_old_tests_and_caches(outdir):
    ...
    # Suppression des fichiers de test .ff, .pyff, etc. (hors test_booster_ia_proj.pyff)
    for dirpath, dirs, files in os.walk(outdir):
        for file_handle in files:
            file_path = os.path.join(dirpath, file_handle)
            # On ne supprime jamais test_booster_ia_proj.pyff
            if (file_handle.endswith('.ff') or file_handle.endswith('.pyff') or file_handle.startswith('test_') or file_handle.endswith('.pyc') or file_handle.endswith('.log') or file_handle.endswith('.bak')) and not file_handle.startswith('test_booster_ia_proj') and file_handle != 'test_booster_ia_proj.pyff':
                try:
                    os.remove(file_path)
    ...
    # Suppression des dossiers __pycache__ et de tous les sous-dossiers, mais jamais le dossier racine
    for dirpath, dirs, files in os.walk(outdir):
...
def clean_macos_files(directory: str):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.startswith('._') or file=='.DS_Store' or file=='Thumbs.db' or file.endswith('.tmp') or file.endswith('.bak') or file.endswith('.log'):
