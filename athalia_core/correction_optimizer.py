...
class CorrectionOptimizer:
        ...
        for i, line in enumerate(lines):
            ...
            if '=' in line and not line.strip().startswith('#'):
                ...
                if 'def ' in line and '=' in line:
                    # Pour les paramètres de fonction, ne corriger que les espaces
                    parts = line.split('=')
                    if len(parts) == 2:
            ...
            # Corriger les espaces dans les parenthèses (mais pas ajouter de parenthèses)
    ...
    def _apply_ast_corrections(self, content: str) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            if error_info['type'] == 'indentation':
                corrected_content, indentation_corrections = self._fix_indentation_error(content, error_info)
                corrections.extend(indentation_corrections)
                return corrected_content, corrections
            
            elif error_info['type'] == 'bracket_balance':
                corrected_content, bracket_corrections = self._fix_bracket_balance(content, error_info)
                corrections.extend(bracket_corrections)
                return corrected_content, corrections
            
            elif error_info['type'] == 'string_issue':
                corrected_content, string_corrections = self._fix_string_issues(content, error_info)
                corrections.extend(string_corrections)
                return corrected_content, corrections
            
            # Si on ne peut pas corriger spécifiquement, essayer les corrections générales
            else:
                # Essayer de corriger les problèmes courants
                corrected_content = content
                
                # Correction des parenthèses manquantes
                corrected_content, bracket_corrections = self._fix_bracket_balance(corrected_content, error_info)
                corrections.extend(bracket_corrections)
                
                # Correction des chaînes
                corrected_content, string_corrections = self._fix_string_issues(corrected_content, error_info)
                corrections.extend(string_corrections)
    ...
    def _apply_contextual_corrections(self, content: str) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        for i, line in enumerate(lines):
            ...
            if context['undefined_variables']:
                keywords = {'def', 'class', 'import', 'from', 'if', 'else', 'for', 'while', 'try', 'except', 'finally', 'with', 'as', 'in', 'is', 'and', 'or', 'not', 'True', 'False', 'None', 'return', 'pass', 'break', 'continue', 'raise', 'yield', 'del', 'global', 'nonlocal', 'assert', 'lambda'}
    ...
    def _fix_indentation_error(self, content: str, error_info: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        if line_num > 0:
            ...
            if prev_line.strip().endswith(':'):
                # Ligne précédente se termine par ':', donc indentation nécessaire
    ...
    def _fix_bracket_balance(self, content: str, error_info: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    def _fix_string_issues(self, content: str, error_info: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    def _analyze_context(self, lines: List[str]) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            for word in words:
                if (word not in context['defined_variables'] and 
                    word not in ['def', 'class', 'import', 'from', 'if', 'else', 'for', 'while', 'try', 'except', 'finally', 'with', 'as', 'in', 'is', 'and', 'or', 'not', 'True', 'False', 'None', 'return', 'pass', 'break', 'continue', 'raise', 'yield', 'del', 'global', 'nonlocal', 'assert', 'lambda']):
                    context['undefined_variables'].add(word)
        
        # Filtrer les variables définies des variables non définies
        context['undefined_variables'] = context['undefined_variables'] - context['defined_variables']
    ...
    def _learn_from_correction(self, file_path: str, original: str, corrected: str, success: bool):
...
def optimize_correction(file_path: str, content: str) -> CorrectionResult:
    ...
    return correction_optimizer.optimize_correction(file_path, content)

def get_correction_stats() -> Dict[str, Any]:
    """Fonction de convenance pour récupérer les statistiques"""
    return correction_optimizer.get_correction_stats() 