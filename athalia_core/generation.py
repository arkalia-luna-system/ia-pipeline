...
def generate_project(blueprint: dict, outdir: str, *args, **kwargs):
    import os
    ...
    return {'outdir': str(outdir), 'files': ['requirements.txt', 'README.md', 'main.py']}

def save_blueprint(blueprint: dict, outdir: str):
    from pathlib import Path
    import yaml
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    with open(outdir / 'blueprint.yaml', 'w', encoding='utf-8') as f:
...
def scan_existing_project(outdir: str):
    from pathlib import Path
    outdir = Path(outdir)
