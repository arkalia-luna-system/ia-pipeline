...
class ArchitectureAnalyzer:
    ...
    def _analyze_all_modules(self) -> Dict[str, ModuleAnalysis]:
        ...
        # Analyser les fichiers Python dans athalia_core (ignorer les fichiers cachés)
    ...
    def _analyze_single_module(self, file_path: Path, module_type: str) -> Optional[ModuleAnalysis]:
    ...
    def _generate_recommendations(self, modules: Dict[str, ModuleAnalysis], 
        ...
        if large_modules:
            recommendations.append(f"📦 {len(large_modules)} modules très grands détectés - considérer la division")
        
        complex_modules = [m for m in modules.values() if m.complexity > 10]
        if complex_modules:
            recommendations.append(f"🧠 {len(complex_modules)} modules complexes - refactoring recommandé")
        
        # Recommandations basées sur les performances
        if performance_issues:
            recommendations.append(f"⚡ {len(performance_issues)} problèmes de performance - optimisation nécessaire")
        
        # Recommandations générales
        if len(modules) > 20:
            recommendations.append("🏗️ Architecture complexe - documenter les dépendances")
    ...
    def get_optimization_plan(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("SELECT name, complexity, performance_score FROM modules WHERE complexity > 10 OR performance_score < 50")
        ...
        return {
            ...
            "optimization_score": max(0, 100 - (avg_complexity * 2 + (100 - avg_performance) * 0.5))
        }
