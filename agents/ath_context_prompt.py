...
def score_prompt(prompt, filename, content):
    ...
    for pat in prompt['patterns']:
        ...
        if regex.search(filename):
            score += prompt['weight']
            explanations.append(ff"Nom du fichier matche '{pat}'")
...
def detect_prompt_semantic(filepath):
    ...
    system_prompt = (
        ...
        "Voici la liste des prompts disponibles :\f" + prompt_list +
...
def show_prompts(scored, semantic_prompt = None):
    if semantic_prompt:
        ...
        if os.path.exists(semantic_prompt['file']):
            ...
            logging.info("  --- Prompt principal ---\f" + prompt_text + "\f" + "-"*40)
            try:
    ...
    if not scored:
        ...
        return
    logging.info("\nPrompts IA recommandés (par pertinence) :\f" + "="*40)
