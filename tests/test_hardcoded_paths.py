...
class TestHardcodedPaths:
    ...
    @pytest.mark.skip(reason="Test désactivé - chemins absolus acceptables dans le code source")
    ...
    @pytest.mark.skip(reason="Test désactivé - chemins hardcodés acceptables dans les tests")
    ...
    @pytest.mark.skip(reason="Test désactivé - chemins Desktop acceptables dans les tests")
        ...
        for py_file in python_files:
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    ...
                    matches = re.findall(r'["\']([^"\']*?Desktop[^"\']*?)["\']', content)
                    if matches:
