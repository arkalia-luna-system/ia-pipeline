...
class TestPerformanceOptimizer:
    ...
    def identify_slow_tests(self, threshold: float = 1.0) -> List[str]:
        ...
        self.slow_tests = [
            test
            for test, duration in self.results.items()
            if duration > threshold
        ]

        print(
            f"🐌 {len(self.slow_tests)} tests lents identifiés (> {threshold}s)"
        )
    ...
    def identify_fast_tests(self, threshold: float = 0.1) -> List[str]:
        ...
        print(
            f"⚡ {len(self.fast_tests)} tests rapides identifiés (< {threshold}s)"
        )
    ...
    def generate_optimization_report(self) -> str:
        ...
        if self.slow_tests:
            ...
            for test in sorted(
                self.slow_tests,
                key=lambda x: self.results.get(x, 0),
                reverse=True,
            ):
        ...
        if self.fast_tests:
            ...
            for test in sorted(
                self.fast_tests, key=lambda x: self.results.get(x, 0)
            ):
                duration = self.results.get(test, 0)
                report.append(f"- `{test}`: {duration:.3f}s")
            report.append("")
        ...
        if self.slow_tests:
            ...
            report.append(
                "3. **Optimiser les setup/teardown** avec `@classmethod`"
            )
    ...
    def save_report(
        self, filename: str = "performance_optimization_report.md"
    ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Optimiseur de performances des tests"
    )
    parser.add_argument(
        "--test-dir", default="tests", help="Répertoire des tests"
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=1.0,
        help="Seuil pour les tests lents (s)",
    )
    parser.add_argument(
        "--report",
