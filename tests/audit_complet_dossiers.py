...
class AuditCompletDossiers:
    ...
    def analyser_tous_dossiers(self) -> List[AuditResult]:
        ...
        for dossier_nom in dossiers_principaux:
            ...
            if dossier_path.exists():
                ...
                result = self._analyser_dossier_complet(
                    dossier_path, dossier_nom
                )
                if result:
        ...
        for sous_dossier in sous_dossiers_caches:
            ...
            result = self._analyser_dossier_complet(
                sous_dossier, sous_dossier.name
            )
            if result:
                results.append(result)
    ...
    def _analyser_dossier_complet(
        ...
        try:
            ...
            dossier_info = self._analyser_dossier_info(
                dossier_path, nom_dossier
            )
    ...
    def _analyser_dossier_info(
        ...
        sous_dossiers = [
            item for item in dossier_path.iterdir() if item.is_dir()
        ]

        # Calculer la taille totale
        taille_totale = sum(
            f.stat().st_size for f in fichiers_python if f.exists()
        )
    ...
    def _analyser_module(self, file_path: Path) -> Optional[ModuleInfo]:
        ...
        try:
            ...
            documentation_associee = self._chercher_documentation_associee(
                file_path
            )

            # Vérifier l'intégration avec l'orchestrateur
            integration_orchestrateur = (
                self._verifier_integration_orchestrateur(content, imports)
            )
    ...
    def _verifier_integration_orchestrateur(
        ...
        if (
            "unified_orchestrator" in imports
            or "intelligent_analyzer" in imports
        ):
            return True
    ...
    def _calculer_score_implementation(
    ...
    def _calculer_score_documentation(
        ...
        total_docs = sum(
            len(module.documentation_associee) for module in modules
        )
        score += min(total_docs * 1.0, 3.0)

        # Points pour les docstrings
        modules_avec_docstring = sum(
            1 for module in modules if module.docstring
        )
        if modules:
    ...
    def _generer_recommandations(
        ...
        modules_non_integres = [
            m for m in modules if not m.integration_orchestrateur
        ]
    ...
    def generer_rapport(self) -> str:
        ...
        for result in sorted(
            self.results, key=lambda x: x.score_total, reverse=True
        ):
            ...
            rapport.append(
                f"**Fichiers MD** : {len(result.dossier.fichiers_md)}"
            )
            rapport.append(
                f"**Sous-dossiers** : {len(result.dossier.sous_dossiers)}"
            )
            rapport.append("")
            ...
            rapport.append(
                f"- **Documentation** : {result.score_documentation:.2f}/10"
            )
            rapport.append(
                f"- **Intégration** : {result.score_integration:.2f}/10"
            )
            rapport.append("")
