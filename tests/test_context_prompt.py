...
class TestPromptScoring:
    ...
    def test_score_prompt_basic(self):
        ...
        score = score_prompt(prompt, filename, content)
...
class TestPromptDetection:
    ...
    def test_detect_prompts_scoring(self, mock_file):
        ...
        assert all(len(p) == 2 for p in scored_prompts)  # (prompt, score)
    ...
    def test_detect_prompts_scoring_with_test_content(self, mock_file):
        ...
        test_prompts = [p for p, score in scored_prompts if "test" in p["name"].lower()]
        assert any(score > 0 for _, score in scored_prompts if any("test" in p["name"].lower() for p, _ in [scored_prompts]))
    ...
    def test_detect_prompts_scoring_with_markdown(self, mock_file):
        ...
        design_prompts = [p for p, score in scored_prompts if "design" in p["name"].lower()]
        assert any(score > 0 for _, score in scored_prompts if any("design" in p["name"].lower() for p, _ in [scored_prompts]))
...
class TestSemanticPromptDetection:
    ...
    def test_detect_prompt_semantic(self, mock_run):
        ...
        assert semantic_prompt in ["test_strategy.md", "code_refactor.yaml", "design_review.md", "ux_fun_boost.md", "dev_debug.yaml"]
...
class TestIntegration:
    ...
    @patch("builtins.open", new_callable=mock_open, read_data="# Documentation\n## Features\n### Security")
    ...
    @patch("builtins.open", new_callable=mock_open, read_data="def refactor_function(): pass")
    def test_refactor_prompt_workflow(self, mock_file):
        ...
        refactor_prompts = [p for p, score in scored_prompts if "refactor" in p["name"].lower()]
