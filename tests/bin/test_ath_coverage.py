...
def test_ath_coverage_runs():
    ...
    try:
        ...
        cleanup_coverage_files()
        assert False, "ath-coverage.py a dépassé le timeout de 30s (test trop long ou bloqué)"
    finally:
        cleanup_coverage_files()
    ...
    assert result.returncode in (0, 1), f"ath-coverage.py a crashé : {result.stderr.decode(errors='ignore')}"
    assert (time.time() - start) < 35, "ath-coverage.py a mis trop de temps à s'exécuter (>30s)" 