...
class TestRequirementsConsistency:
    ...
    def test_essential_dependencies(self):
        ...
        try:
            ...
            if missing_deps:
                pytest.fail(
                    f"Dépendances essentielles manquantes: {missing_deps}")
    ...
    def test_no_duplicate_dependencies(self):
        ...
        if duplicates:
            pytest.fail(
                f"Dépendances dupliquées: {duplicates}"
            )
    ...
    def test_pyproject_toml_exists(self):
        ...
        assert pyproject_file.exists(), (
            "Fichier pyproject.toml manquant")
    ...
    def test_no_obsolete_dependencies(self):
        ...
        try:
            ...
            if found_obsolete:
                pytest.fail(
                    f"Dépendances obsolètes trouvées: {found_obsolete}")
                ...
                if line and not line.startswith('#'):
                    ...
                    if '==' in line:
                        ...
                        assert parts[0].strip(), (
                            f"Ligne {i}: nom de package manquant")
                        assert parts[1].strip(), (
                            f"Ligne {i}: version manquante")
