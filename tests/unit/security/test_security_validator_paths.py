...
class TestCommandSecurityValidatorPaths:
    ...
    def test_no_macos_specific_paths(self):
        """Vérifie qu'aucun chemin macOS hardcodé n'est présent (interpréteur actuel autorisé)."""
        import os
        import sys

        # Sur macOS avec pyenv/homebrew, sys.executable est dans allowed_commands par design
        if sys.executable and (
            "homebrew" in sys.executable or "pyenv" in sys.executable
        ):
            pytest.skip("Sur macOS/pyenv l'exécutable actuel est autorisé par design")

        validator = CommandSecurityValidator()
        current_executable = sys.executable or ""
        current_bin_dir = (
            os.path.dirname(current_executable) if current_executable else ""
        )

        # Chemins macOS hardcodés à éviter; autoriser le répertoire de l'interpréteur actuel
        forbidden_paths = [
        ...
        for forbidden_path in forbidden_paths:
            if forbidden_path == current_executable:
                continue
            if current_bin_dir and forbidden_path.startswith(current_bin_dir):
                continue
            assert (
    ...
    def test_no_absolute_paths_in_whitelist(self):
        """Vérifie qu'aucun chemin absolu hardcodé n'est présent (interpréteur actuel autorisé)."""
        import os
        import sys

        # Sur macOS avec pyenv/homebrew, l'exécutable actuel est dans la whitelist par design
        if sys.executable and (
            "homebrew" in sys.executable or "pyenv" in sys.executable
        ):
            pytest.skip("Sur macOS/pyenv l'exécutable actuel est autorisé par design")
        ...
        validator = CommandSecurityValidator()
        current_bin_dir = os.path.dirname(sys.executable or "") or ""

        for command in validator.allowed_commands:
            if "/opt/homebrew/opt/pyenv/versions/" not in command:
                continue
            if current_bin_dir and command.startswith(current_bin_dir):
                continue
