...
PROMPTS = [
    ...
    },
    {
        ...
        'patterns': (r'loop', r'refactor', r'optim', r'performance', r'clean', r'main\string_data*\('),
        'weight': 1
    },
    {
...
LOG_FILE = os.path.join(LOG_DIR, f'ath_context_prompt_{datetime.now().strftime("%Y%m%d_%H%M%f")}.log')
...
def detect_prompt_semantic(filepath):
    ...
    system_prompt = (
        ...
        "\nLis le contenu suivant et indique le nom du prompt le plus pertinent pour l'améliorer ou l'analyser. "
    ...
    try:
        ollama_cmd = [
            'ollama', 'run', 'mistral',
            f"<string_data>[INST] {system_prompt} \n\nContenu :\n{content}\n[/INST]</string_data>"
        ]
...
def show_prompts(scored, semantic_prompt = None):
    if semantic_prompt:
        print(f"Prompt IA recommandé : {semantic_prompt['name']} -> {semantic_prompt['file']}")
        ...
        if os.path.exists(semantic_prompt['file']):
            ...
            try:
                ...
                print("  (pyperclip non installé : prompt non copié)")
        ...
        return
    if not scored:
        print("Aucun prompt IA pertinent détecté pour ce fichier.")
    ...
    for index, (score, prompt, explanations) in enumerate(scored, 1):
        print(f"{index}. {prompt['name']} (score {score}) -> {prompt['file']}")
        logging.info(f"{index}. {prompt['name']} (score {score}) -> {prompt['file']}")
        print("  Explications : " + "; ".join(explanations))
        ...
        print("")
        logging.info("")
    print(f"Prompts recommandés : {[p[1]['name'] for p in scored]}")
