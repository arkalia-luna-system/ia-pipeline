...
PROMPTS = [
    ...
    },
    {
        ...
        'patterns': [r'loop', r'refactor', r'optim', r'performance', r'clean', r'main\s*\('],
        'weight': 1
    },
    {
...
LOG_FILE = os.path.join(LOG_DIR, f'ath_context_prompt_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
def show_prompts(scored, semantic_prompt=None):
    if semantic_prompt:
        ...
        if os.path.exists(semantic_prompt['file']):
            with open(semantic_prompt['file'], 'r', encoding='utf-8') as f:
                prompt_text = f.read()
    ...
    for i, (score, prompt, explanations) in enumerate(scored, 1):
        print(f"{i}. {prompt['name']} (score {score}) -> {prompt['file']}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage : ath_context_prompt.py <fichier1> [<fichier2> ...]")
        sys.exit(1)
