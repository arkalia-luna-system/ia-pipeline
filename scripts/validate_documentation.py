...
class DocumentationValidator:
                ...
                self.issues.append(
                    {
                        ...
                        "message": (
                            f"Mermaid diagrams not properly closed ({mermaid_count} opened)"
                        ),
                        "severity": "high",
                    }
                )
    ...
    def _calculate_quality_score(self):
        ...
        high_severity_penalty = (
            len([i for i in self.issues if i["severity"] == "high"]) * 5
        )
        medium_severity_penalty = (
            len([i for i in self.issues if i["severity"] == "medium"]) * 2
        )
        low_severity_penalty = (
            len([i for i in self.issues if i["severity"] == "low"]) * 0.5
        )

        total_penalty = (
            high_severity_penalty + medium_severity_penalty + low_severity_penalty
        )
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        total_files = self.stats["total_files"]
        diagram_ratio = (
            self.stats["files_with_diagrams"] / total_files if total_files > 0 else 0
        )
        code_ratio = (
            self.stats["files_with_code"] / total_files if total_files > 0 else 0
        )
        badge_ratio = (
            self.stats["files_with_badges"] / total_files if total_files > 0 else 0
        )
        ...
        if code_ratio < 0.4:
            recommendations.append(
                "🟡 Include more code examples (target: 40%+ of files)"
            )
        ...
        if self.stats["broken_links"] > 0:
            recommendations.append(
                f"🔴 Fix {self.stats['broken_links']} broken internal links"
            )
...
"""
    ...
    print("   - Markdown: doc_quality_report.md")
...
def main():
    ...
    parser.add_argument(
        "--output", default="doc_quality_report.json", help="Output report file"
    )
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix common issues"
    )
    parser.add_argument(
        "--threshold", type=float, default=80.0, help="Quality threshold for pass/fail"
    )
