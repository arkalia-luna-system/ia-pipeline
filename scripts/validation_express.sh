...
timeout 5s python bin/athalia_unified.py --help > /dev/null 2>&1
...
timeout 15s python bin/athalia_unified.py "$PROJECT_TEST" --action complete > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
timeout 5s python bin/athalia_unified.py "$(dirname "$TEST_FILE")" --action fix --auto-fix > /dev/null 2>&1
if [ $? -eq 0 ]; then
