...
class DocumentationAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            has_obsolete_refs = bool(
                re.search(r"SPECIALIZED|old_|deprecated", content, re.IGNORECASE)
            )
            ...
            issues = []
            if has_old_dates:
                issues.append("Dates obsolètes")
            if has_obsolete_refs:
                issues.append("Références obsolètes")
            if has_broken_links:
                issues.append("Liens potentiellement cassés")
            if total_lines < 5:
                issues.append("Fichier trop court")
                ...
                recommendations["improve"].append(
                    {
                        "file": file_path,
                        "reason": (
                            f"Qualité correcte mais améliorable (score: {quality_score})"
                        ),
                        "issues": issues,
                    }
                )
                ...
                recommendations["consolidate"].append(
                    {
                        "file": file_path,
                        "reason": (
                            f"À consolider avec d'autres fichiers (score: {quality_score})"
                        ),
                    }
                )
    ...
    def generate_report(self) -> str:
        ...
        quality_scores = [
            ...
            for a in self.analysis_results.values()
            if "error" not in a
        ]
        ...
        report.append(
            f"- **Fichiers excellents (≥80) :** {len([s for s in quality_scores if s >= 80])}"
        )
        report.append(
            f"- **Fichiers corrects (60-79) :** {len([s for s in quality_scores if 60 <= s < 80])}"
        )
        report.append(
            f"- **Fichiers faibles (<60) :** {len([s for s in quality_scores if s < 60])}"
        )
        report.append("")
