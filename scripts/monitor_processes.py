...
class AthaliaProcessMonitor:
    ...
    def kill_duplicate_processes(self, keep_oldest: bool = True) -> int:
        ...
        for proc_info in to_kill:
            try:
                ...
                self.logger.error(
                    f"❌ Erreur lors de l'arrêt du processus {proc_info['pid']}: {e}"
                )
    ...
    def monitor_processes(self, interval: int = 30, max_duration: int = 3600):
        ...
        while time.time() - start_time < max_duration:
            ...
            if len(processes) > 1:
                self.logger.warning(
                    f"⚠️ {len(processes)} processus athalia_core.main détectés"
                )
    ...
    def get_process_stats(self) -> Dict[str, Any]:
        ...
        if not processes:
            return {"count": 0, "total_cpu": 0, "total_memory": 0, "status": "inactive"}
