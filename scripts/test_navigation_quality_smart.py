...
class SmartNavigationQualityTester:
    ...
    def is_valid_file_reference(self, url: str, source_file: Path) -> bool:
        ...
        try:
            ...
            if url in ['README.md', 'CHANGELOG.md', 'INVENTAIRE_COMPLET.md', 
                       'RAPPORT_FINAL.md', 'FINAL_SUMMARY.md', 'GENESIS.md',
                       'CLEANUP_REPORT.md', 'FAQ.md', 'INSTALL.md']:
        ...
        try:
            ...
            for file_ref in file_refs:
                links.append({
                    ...
                    'text': file_ref,
                    'url': file_ref,
                    'line': content[:content.find(f'`{file_ref}`')].count('\n') + 1
                })
    ...
    def test_file_navigation_smart(self, file_path: Path) -> Dict:
        ...
        valid_links = len([l for l in validated_links if l['is_valid']])
        critical_links = len([l for l in validated_links if l['is_critical']])
        valid_critical_links = len([l for l in validated_links if l['is_valid'] and l['is_critical']])
        
        # Score pondéré : liens critiques comptent plus
        if total_links > 0:
            if critical_links > 0:
                critical_score = (valid_critical_links / critical_links) * 60  # 60% du score
                if (total_links - critical_links) > 0:
                    other_score = ((valid_links - valid_critical_links) / (total_links - critical_links)) * 40  # 40% du score
                else:
                    other_score = 0
    ...
    def test_all_files_smart(self) -> Dict:
        ...
        link_success_rate = ((total_links - total_broken) / total_links * 100) if total_links > 0 else 100
    ...
    def generate_smart_report(self) -> str:
        ...
        report = []
        report.append("# 🔗 RAPPORT INTELLIGENT DE TEST NAVIGATION DOCUMENTATION ATHALIA")
        ...
        report.append(f"- **Liens critiques cassés :** {global_stats['critical_broken_links']}")
        report.append(f"- **Score de navigation moyen :** {global_stats['average_navigation_score']:.1f}/100")
        report.append(f"- **Taux de succès des liens :** {global_stats['link_success_rate']:.1f}%")
        report.append("")
        
        # Fichiers avec problèmes critiques
        critical_problematic_files = [f for f, r in self.navigation_results.items() 
        ...
        if critical_problematic_files:
            ...
            for file_path in critical_problematic_files[:10]:  # Top 10
                ...
                report.append(f"- **`{file_path}`** - {result['critical_broken_links']} liens critiques cassés (score: {result['navigation_score']:.1f}/100)")
        ...
        perfect_files = [f for f, r in self.navigation_results.items() if r['navigation_score'] == 100]
        
        if perfect_files:
            report.append("## ✅ FICHIERS AVEC NAVIGATION PARFAITE")
            for file_path in perfect_files[:10]:  # Top 10
                result = self.navigation_results[file_path]
                report.append(f"- **`{file_path}`** - {result['total_links']} liens valides")
            if len(perfect_files) > 10:
                report.append(f"- ... et {len(perfect_files) - 10} autres")
            report.append("")
        
        # Détails des liens critiques cassés
        critical_broken = [l for l in self.broken_links if l['is_critical']]
        if critical_broken:
            report.append("## 🔧 DÉTAILS DES LIENS CRITIQUES CASSÉS")
            for broken_link in critical_broken[:20]:  # Top 20
                report.append(f"- **`{broken_link['source_file']}`** (ligne {broken_link['link']['line']})")
                report.append(f"  - Lien : `{broken_link['link']['text']}` → `{broken_link['link']['url']}`")
                report.append(f"  - Erreur : {broken_link['error_type']}")
                report.append(f"  - Solution : {broken_link['suggested_fix']}")
                report.append("")
            if len(critical_broken) > 20:
                report.append(f"- ... et {len(critical_broken) - 20} autres liens critiques cassés")
        
        return "\n".join(report)
    
    def save_smart_results(self, output_file: str = "navigation_test_smart_results.json"):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                'critical_broken_links': [l for l in self.broken_links if l['is_critical']]
