...
def split_api_documentation():
    ...
    print(
        f"📖 Lecture du fichier API.md ({api_file.stat().st_size / (1024*1024):.1f}MB)"
    )
    ...
    for line in lines:
        ...
        if line.startswith("# "):
            ...
            if "overview" in line.lower() or "vue d'ensemble" in line.lower():
                ...
                util in line.lower() for util in ["utility", "helper", "tool"]
            ):
...
"""
    ...
    file_mapping = {
        ...
        "core_modules": (
            "core_modules.md",
            "Modules principaux (analytics, audit, cleaner, etc.)",
        ),
        "orchestrator": (
            "orchestrator.md",
            "Orchestrateur unifié et pipeline",
        ),
