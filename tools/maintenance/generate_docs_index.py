...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            "logs/index_generation.log", mode="a", encoding="utf-8"
        ),
...
class IndexGenerator:
    ...
    def _generate_main_index(self) -> str:
        ...
        content += f"**Date de génération :** {datetime.now().strftime('%d/%m/%Y à %H:%M')}\n"
        ...
        content += f"- **Robotics :** {len(self.index_structure['robotics'])} fichiers\n\n"
        ...
        content += "- **Contribution** → [GUIDES/CONTRIBUTING.md](GUIDES/CONTRIBUTING.md)\n\n"
...
def main():
    ...
    print(f"📊 Fichiers indexés : {sum(len(items) for items in generator.index_structure.values())}")
