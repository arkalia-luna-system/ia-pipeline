...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/workspace_organizer.log', mode='a', encoding='utf-8'),
...
class WorkspaceOrganizer:
    ...
    def _load_organization_rules(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "root_files": [
                ...
                "pytest.ini", "activate_venv.sh", "athalia_unified.py", 
                "athalia.f(f", ".gitignore", ".coverage", ".pytestignore"
            ],
            
            # Scripts de démonstration et validation
            "scripts": [
                "demo_*.py", "validation_*.py", "test_logging_*.py"
            ],
                ...
                for file_path in files:
                    ...
                    if dry_run:
                        ...
                        try:
                            ...
                            logger.error(f"❌ Erreur lors du déplacement de {file_path}: {e}")
        
        return {"moved_files": moved_files, "files_by_category": files_by_category}
    
    def generate_organization_report(self, files_by_category: Dict[str, List[Path]]) -> str:
    ...
    def validate_organization(self) -> Dict[str, bool]:
        ...
        essential_dirs = [
            "scripts", "tools", "bin", "data", "dashboard", "docs", "tests"
        ]
        ...
        essential_files = [
            "README.md", "LICENSE", "requirements.txt", "athalia_unified.py"
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Organisateur de Workspace Athalia")
    parser.add_argument("--root", default=".", help="Chemin racine du workspace")
    parser.add_argument("--dry-run", action="store_true", help="Mode simulation")
    parser.add_argument("--cleanup", action="store_true", help="Nettoyer les fichiers temporaires")
    parser.add_argument("--validate", action="store_true", help="Valider l'organisation")
    ...
    if args.report:
        ...
        report_path = Path("logs") / f"workspace_organization_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
